...
import { Patient, SurgeryGroup, createInitialAudiometry, createInitialOtoscopy, calculateAverage, calculateABG, AudiometryResult, Frequencies } from '../types';
...
import AudiogramPlotter from './AudiogramPlotter';
...
const emptyPatient: Patient = {
    ...
    diagnosis: '',
    preOpOtoscopy: { right: createInitialOtoscopy(), left: createInitialOtoscopy() },
    preOpAudiometry: createInitialAudiometry(),
    postOp6WeeksOtoscopy: { right: createInitialOtoscopy(), left: createInitialOtoscopy() },
    postOp6WeeksAudiometry: createInitialAudiometry(),
    postOp12WeeksOtoscopy: { right: createInitialOtoscopy(), left: createInitialOtoscopy() },
    postOp12WeeksAudiometry: createInitialAudiometry(),
};
            ...
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
                ...
                <div className="flex flex-col xl:flex-row gap-6">
                    ...
                    <div className="flex-1">
                        <AudiogramPlotter
                            airConduction={data.airConduction}
                            boneConduction={data.boneConduction}
                            side={side}
                            onChange={(type, freq, value) => updateAudiometry(phase, side, type, freq, value)}
                        />
                    </div>
                    ...
                    <div className="w-full xl:w-48 flex flex-col justify-center">
                        <div className="grid grid-cols-2 gap-x-3 gap-y-2 mb-4">
                            <span className="col-span-2 text-xs font-bold text-gray-400 uppercase tracking-wider text-center mb-1">Air Conduction</span>
                            {(['f500', 'f1k', 'f2k', 'f4k'] as const).map(f => (
                                <div key={`ac-${f}`} className="relative">
                                    <span className="absolute left-2 top-1.5 text-[10px] text-gray-400 font-mono">{f.replace('f', '')}</span>
                                    <input
                                        type="number"
                                        className={`${gridInputClass} pl-8`}
                        ...
                        <div className="grid grid-cols-2 gap-x-3 gap-y-2">
                            <span className="col-span-2 text-xs font-bold text-gray-400 uppercase tracking-wider text-center mb-1">Bone Conduction</span>
                            {(['f500', 'f1k', 'f2k', 'f4k'] as const).map(f => (
                                <div key={`bc-${f}`} className="relative">
                                    <span className="absolute left-2 top-1.5 text-[10px] text-gray-400 font-mono">{f.replace('f', '')}</span>
                                    <input
                                        type="number"
                                        className={`${gridInputClass} pl-8`}
            ...
            <div className="bg-white p-1.5 rounded-xl shadow-sm border border-gray-200 flex space-x-1 overflow-x-auto sticky top-0 z-10 backdrop-blur-md bg-white/80">
                {[
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center space-x-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 whitespace-nowrap outline-none focus:ring-2 focus:ring-teal-500/20 ${activeTab === tab.id
